...
function install_loading_indicator() {
	...
	if (location.search.match("name="))
		...
		txt = location.pathname.replace(/.*\//, "").replace(/\..*/, "");
	elt.innerHTML = txt;
	div.appendChild(elt);
	elt = create_spinner();
	div.appendChild(elt);
...
function get_params() {
	var spot_data = location.pathname
				.replace(/.*\//, "data/")
				.replace(/\..*/, ".data");
	return {
		data: get_var(spot_data),	// may be online
		opts: get_var("opts.js"),	// in local cache
		lang: WgLang  			// in lang.js
	};
}
